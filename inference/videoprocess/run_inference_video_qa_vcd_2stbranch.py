...
def get_model_output(model, video_processor, tokenizer, video, video_temcd, video_spacd, qs, args):
    ...
    if args.use_temcd:
            ...
            video_tensor_cd = video_processor.preprocess(frames, return_tensors='pt')['pixel_values'][0].half().to(args.device)
    else:
            video_tensor_cd = None   
            
    if args.use_spacd:
            print(f'begining spa cd function...')
            video_tensor_spacd = video_processor.preprocess(frames, return_tensors='pt')['pixel_values'][0].half().to(args.device)
...
def run_inference(args):
    ...
    for sample in tqdm(gt_questions):
        ...
        for fmt in tqdm(video_formats):  # Added this line
            ...
            video_temcd_dir = os.path.join(args.video_temcd_dir, f"{video_name}{fmt}.mp4")
            if os.path.exists(temp_path):
                video_path = temp_path
                if os.path.exists(mask_temp_path):
                    video_temcd_path = video_temcd_dir
                if os.path.exists(mask_temp_path):
                    video_spacd_path = mask_temp_path

                print(f'video_temcd_dir:{video_temcd_path}, video_spacd_dir:{video_spacd_path}')
                # try:
                # Run inference on the video and add the output to the list
                output = get_model_output(model, video_processor, tokenizer, video_path, video_temcd_path, video_spacd_path, question, args)
